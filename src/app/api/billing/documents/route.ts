...
export async function GET() {
  try {
    ...
    const hasQuotations = documents.some(doc => doc.type === 'Quotation');

    if (documents.length === 0 || !hasQuotations) {
      // Seed missing default documents
      const docsToSeed = documents.length === 0 
        ? DEFAULT_DOCUMENTS 
        : DEFAULT_DOCUMENTS.filter(doc => doc.type === 'Quotation');

      await Promise.all(
        docsToSeed.map((doc) =>
          prisma.billingDocument.upsert({
            where: { id: doc.id },
            update: {},
            create: {
              ...doc,
