...
export const BorderGlow: React.FC<BorderGlowProps> = ({
  ...
  useEffect(() => {
    ...
    animateValue({ ease: easeInCubic, duration: 1500, end: 50, onUpdate: v => {
      card.style.setProperty('--cursor-angle', `${((angleEnd - angleStart) * (v / 100) + angleStart).toString()}deg`);
    }});
  ...
  const borderColorVar = isLight ? 'rgba(0, 0, 0, 0.08)' : 'rgb(255 255 255 / 15%)';
